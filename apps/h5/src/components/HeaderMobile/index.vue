<template>
  <div class="wrapper flex justify-between items-center">
    <div>
      ...
      <img class="ml-20 h-45" :src="Logo" alt="" @click="linkTo('/')" />
    </div>
    <!-- <SvgIcon v-show="showSetting" name="bx-icon-mobile-header-setting" class="right-icon" @click="showRate" /> -->
    <div class="right">
      <!-- 登录与未登录状态 -->
      <div v-show="isLogin" class="flex items-center">
        <div class="pl-10 pr-20 py-4 flex justify-between items-center w-250 border-6 border-solid border-#ea4b64 rounded-40">
          <div class="flex items-center">
            <img class="w-24 h-auto" :src="Coin" alt="" />
            <span class="ml-10">{{ userStore?.userInfoDetail.gold_coin }}</span>
          </div>
        ...
        <Popover v-model:show="showPopover" placement="bottom-end">
          <div v-if="isLogin" class="p-5 bg-#ffffff text-20 text-#000000">
            <div v-for="item in actions2" :key="item.key" class="m-5" @click="item.handle">{{ item.text }}</div>
          </div>
          <div v-else class="p-5 bg-#ffffff text-20 text-#000000">
            <div v-for="item in actions" :key="item.key" class="m-5" @click="linkTo(item.path)">{{ item.text }}</div>
          </div>
          <template #reference>
            <div class="ml-20 flex justify-center items-center w-50 h-50 rounded-50% bg-#ea4b64">
              <img class="w-35 h-auto" :src="Person" alt="" />
            </div>
          </template>
        </Popover>
      </div>
      <Popover v-if="!isLogin" v-model:show="showPopover" placement="bottom-end">
        <div class="p-5 bg-#ffffff text-20 text-#000000">
          <div v-for="item in actions" :key="item.key" class="m-5" @click="linkTo(item.path)">{{ item.text }}</div>
        </div>
        <template #reference>
          <div class="ml-20 flex justify-center items-center w-50 h-50 rounded-50% bg-#ea4b64">
  ...
  <van-popup v-model:show="show" round>
    <div class="py-20 px-40 w-90vw h-75vh bg-#202020 rounded-20">
      <div class="text-center text-32"> 储值 </div>
      <div class="mt-20 p-20 bg-#373737 rounded-10">
        <div class="text-#919191">当前持有</div>
        <div class="mt-15 flex items-center">
          <img class="w-24 h-auto" :src="Coin" alt="" />
          <span class="ml-10 text-32 font-black">{{ userStore?.userInfoDetail.gold_coin }}</span>
        </div>
      </div>
      <div class="mt-20 p-20 bg-#373737 rounded-10">
        <div class="text-#919191">充值方案</div>
        <div class="mt-20 flex overflow-x-auto hide-scrollbar text-22">
          <div
            v-for="(item, index) in goods"
            :key="item.id"
            :class="[
              'mr-15 p-20 min-w-170 rounded-10 bg-#151515 box-content font-black',
              { 'border-6 border-solid border-#ea4b64': defaultCoinType == index },
            ]"
            @click="handleChangeCoinType(index)"
          >
            <div class="text-center">
              <img class="w-auto h-60" :src="getCoinImg(index)" alt="" />
            </div>
            <div class="mt-25 flex items-center">
              <img class="w-24 h-auto" :src="Coin" alt="" />
              <span class="ml-10">+ {{ item.gold_coin }}</span>
            </div>
            <div class="mt-15 flex items-center">
              <img class="w-24 h-auto" :src="Silver" alt="" />
              <span class="ml-10">+ {{ item.silver_coin }}</span>
            </div>
            <div class="mt-20 text-24 text-center">
              CNY <span>{{ item.price }}</span>
            </div>
          </div>
        </div>
        <div class="mt-20 text-22 text-#ea4b64"> 上方显示的金额可能与实际支付页面金额显示不同，请以支付页面金额为该支付金额。 </div>
      </div>
      <div class="mt-20 p-20 bg-#373737 rounded-10">
        <div class="text-#919191">支付</div>
        <div class="mt-20 text-22">
          <van-radio-group v-model="defaultChannel" shape="dot" direction="horizontal">
            <van-radio v-for="(item, index) in channel" :key="item.id" :name="index">{{ item.name }}</van-radio>
          </van-radio-group>
        </div>
      </div>
      <div class="mt-20 p-20 bg-#373737 rounded-10">
        ...
        <div class="flex flex-col justify-center items-center w-180" @click="handlePay">
          <div
            class="mt-20 flex flex-col justify-center items-center w-full h-170 border-6 border-solid border-#4b4b4b inline-block rounded-10 bg-#414141"
          >
            <img class="w-140 h-auto" :src="PaymentAlipay" alt="" />
            <div class="mt-20 text-#919191 text-24">CNY</div>
          </div>
          <div class="mt-20 py-4 px-30 rounded-40 border-3 border-solid border-#ea4b64 text-#ea4b64 inline-block text-22">前往支付</div>
        </div>
      </div>
    </div>
  </van-popup>
...
<script lang="ts" setup name="header-mobile">
  ...
  const actions = [
    { text: '注册账号', key: 'register', path: '/register' },
    { text: '登入', key: 'login', path: '/login' },
