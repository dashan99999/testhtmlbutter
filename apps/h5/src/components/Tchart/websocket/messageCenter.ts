...
export class MessageCenter {
  ...
  // 批量销毁调度中心中某个函数集
  unHandler(type: string, handler: any) {
   ...
   * @param handler 事件钩子
   */
  checkHandler(type: string, handler: () => void) {
    if (type?.length === 0) {
...
export const messageCenter: MessageCenter =
