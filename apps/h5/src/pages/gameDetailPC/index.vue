<template>
  <div class="px-1/4 pb-140 text-#ffffff bg-#212121">
    <div class="relative min-h-240">
      <img class="w-full h-auto" :src="detail.cover_image" alt="" />
      <div
        class="absolute left-1/3 top-2/5 py-10 px-40 rounded-60 border-2 border-solid border-white inline-block bg-black bg-opacity-80 font-extrabold animate-bounce"
      >
        立即游玩
      </div>
    </div>
    <div class="py-0">
      <div class="grid gap-8" :style="{ gridTemplateColumns: '1fr 2fr 3fr' }">
        <div class="flex flex-col justify-center items-center h-50 rounded-10 bg-#2c2c2c">
          <div>-</div>
          <div class="mt-10 text-12 text-#919191">好评数</div>
        </div>
        <div class="flex flex-col justify-center items-center h-50 rounded-10 bg-#2c2c2c">
          <div class="flex items-center">
            <div v-for="platform in detail.game_link_obj" :key="platform.id">
              ...
              <img v-if="platform.device_type == 1" class="mx-10 w-14 h-auto" :src="Platform5" alt="" />
              <img v-if="platform.device_type == 2" class="w-14 h-auto" :src="Platform6" alt="" />
            </div>
          </div>
          <div class="mt-10 text-12 text-#919191">支持设备</div>
        </div>
        <div class="flex flex-col justify-center items-center h-50 rounded-10 bg-#2c2c2c">
          <div>{{ detail.author }}</div>
          <div class="mt-10 text-12 text-#919191">发行商</div>
        </div>
      </div>
      <div class="mt-8 px-10 flex flex-col justify-center items-center h-60 rounded-10 bg-#2c2c2c text-center">
        <!-- <div class="font-bold"> 简体中文 及其他 5 个语言 </div> -->
        <div class="w-full truncate whitespace-nowrap font-bold"> {{ detail.language_Support }} {{ detail.language_Support }} </div>
        <div class="mt-5 text-12 text-#919191"> 支持语言 </div>
      </div>
      <div class="py-20">
        <div class="text-20 font-black">{{ detail.name }}</div>
        <div class="pt-10 text-12 font-normal">
          {{ detail.introduce }}
        </div>
      </div>
      <div class="flex justify-start">
        <div
          ...
          :key="label.id"
          class="mr-5 px-4 py-2 text-10 border-1 border-solid border-#919191 text-#919191 rounded-6 inline-block"
        >
          {{ label }}
        </div>
      </div>
      ...
      <div class="mt-20">
        ...
        >
          <swiper-slide v-for="(item, index) in detail.preview_image" :key="index" class="rounded-20">
            <img class="w-full h-auto" :src="item" />
          </swiper-slide>
        </swiper>
      </div>
      ...
      <div class="p-40">
        <div class="flex justify-start">
          <div class="flex flex-col justify-center items-center">
            <div class="text-18 font-normal">游戏资讯</div>
            <div class="w-34 h-6 rounded-8 bg-#ea4b64"></div>
          </div>
        </div>

        <div class="mt-10 text-12 text-f5f5f5 leading-20">{{ detail.information }}</div>
      </div>
    </div>

    <div class="absolute bottom-0 left-0 right-0 z-9999 bg-#202020">
      <div class="px-1/4 py-5 flex items-center h-30">
        <div class="flex-1 flex justify-center items-center h-full bg-#ea4b64 rounded-80 text-12 font-bold" @click="handleOpenBuy">
          <span>马上购买</span>
          <img class="mx-20 w-14 h-auto" src="https://playmeow.com/_nuxt/img/icon_coin.17301a8.svg" alt="" />
          <span>{{ detail.price }}</span>
        </div>
        <div class="ml-20"><img class="w-18 h-18" :src="More" alt="" @click="handleShowDownload" /></div>
      </div>
      <div v-if="showDownload" class="px-1/4 pb-2 text-#919191 text-12">
        <div class="mb-4">以下是试玩版本的下载链结</div>
        <div v-for="platform in detail.game_link_obj" :key="platform.id">
          <div v-if="platform.device_type == 1" class="flex items-center">
            <img class="w-15 h-15" :src="Platform8" alt="" />
            <a :href="platform.game_link" download="win_game.zip">
              <div class="mx-5 px-3 py-3 text-12 border-1 border-solid border-#919191 text-#919191 rounded-3">立即下载</div>
            </a>
            <!-- <div class=""> 点击链接可下载游玩 </div> -->
          </div>
          <div v-if="platform.device_type == 2" class="mt-20 flex items-center">
            <img class="w-15 h-15" :src="Platform7" alt="" />
    ...
    <van-popup v-model:show="buyPopup" round>
      <div class="py-20 px-40 w-90vw h-55vh bg-#202020 rounded-20 text-12">
        ...
        <div class="mt-60 p-20 bg-#373737 rounded-10">
          <div class="h-350">
            ...
            <div class="mt-15 flex items-center">
              ...
              <span class="ml-10 text-32 font-black text-12">{{ detail?.price }}</span>
            </div>
          </div>

          <div class="mt-15 px-20 py-20 flex justify-between items-center border-y-1 border-y-dashed border-#919191">
            <div>总额:</div>
            <div class="ml-10 text-32 font-black text-12">{{ detail?.price }}</div>
          </div>
          <div class="mt-15 px-20 pb-20 border-b-1 border-b-dashed border-#919191">
            <div class="flex justify-between items-center">
              ...
              >
              <div class="ml-10 text-32 font-black text-12">{{ userStore?.userInfoDetail.silver_coin }}</div>
            </div>
