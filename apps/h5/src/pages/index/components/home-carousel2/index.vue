<template>
  <div>
    <div class="my-20 px-20">
      <div class="text-40 font-bold">最新游戏</div>
    </div>
    <swiper
      :slidesPerView="2"
      :spaceBetween="30"
    ...
    >
      <swiper-slide v-for="(item, index) in games?.latest" :key="index" class="rounded-20">
        <GameCard :game="item" :index="index" @on-expand="onExpand('latest', index)" />
      </swiper-slide>
    </swiper>

    <div class="my-20 mt-60 px-20">
      <div class="text-40 font-bold">畅销游戏</div>
    </div>
    ...
    <div class="my-20 mt-60 px-20">
      <div class="text-40 font-bold">推荐游戏</div>
    </div>
    <div class="px-20 grid grid-cols-2 gap-20">
      <GameCard
